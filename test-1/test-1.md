...
# 2. Export Application Related Manifests

Based on the Namespaces procured from the step above run the following loops to derive different object configurations. Eventhough all the configuratiions are covered below, it is always recommended to take these instructions as base to design the migration mechanism.
...
# 3. Image Migration from OpenShift Internal Registry to ECR or any other Conformant Registry (Based on current practices)
...
While you can use any container registry as the target container registry, it is recommended to use a OCI compliant registry like ECR (Elastic Container Registry) a managed image registry on AWS. 
...
## Connecting to Target Image Registry (that is intended to be used with EKS) using the allowed method 

For example, if using ECR
...
* For more details on using ECR with EKS: https://docs.aws.amazon.com/AmazonECR/latest/userguide/getting-started-cli.html

## Migrate Images for an Application

For the application on the namespace you are trying to migrate, you can migrate the container images from source OpenShift Internal Registry to the target 
...
```
TARGET_REPO="aws_account_id.dkr.ecr.region.amazonaws.com\/<<your-project>>"
...
If you want to migrate all the container images for the selected namespaces from source OpenShift Internal Registry to target ECR registry instead of doing it on a per namespace basis, run this script. This script may take several hours to run depending on the number of images to be migrated, their size and your connection speed to the source and target registries.
...
# 4. Migrate Applications without Persistent Storage (Stateless Applications)
...
We will be using [Shifter tool](https://github.com/google/shifter/releases) to convert the manifests from [OpenShift format to Kubernetes format]. Download the latest release of Shifter [from here](https://github.com/google/shifter/releases) onto your linux box.

```
# Download shifter CLI
https://github.com/google/shifter/releases 
wget
https://github.com/google/shifter/releases/download/v0.3.0/shifter_linux_amd64
...
The target cluster may require credentials to pull images from a container registry, if the registry is protected. In this section, we will see how to set up a secret to the container registry.

To provide access to your application deployment to pull images from this registry, create a service account that can pull images from the private registry.

For example, with ECR create a registry secret within the above namespace that would be used to pull an image from a private ECR repository
```
...
* **If the images have been migrated** to [a different registry as part of migration] from OpenShift Internal Registry, change the container image in the deployment to point to the target repository where the image is moved.
...
* Connect to the Target EKS Cluster, if you are not already connected

```
aws eks update-kubeconfig --name example
```

For more information refert to: https://docs.aws.amazon.com/cli/latest/reference/eks/update-kubeconfig.html
...
# 5. Migrate Applications with Persistent Storage using Velero

This section further expands the migration capability to include both the persistent data and workloads. From previous sections, it is eviddent that Openshift has several propietary objects (e.g. DeploymentConfig, Route), which are different from Kubernetes. Luckily, when it comes to Persistent Data, Openshift sticks with Kubernetes standard. Therefore, we could achieve the migration with the help of Opensource data migration tools. 

In this demo, we will use [Velero](https://velero.io/) as an example. However, there are other 3rd-party tools (e.g. Kasten from Veeam, Portworx) could also be used. Readers could select one which fits your work. Like our earlier work, we will [Shifter](https://github.com/google/shifter/releases) for workloads migration. Have both Velero CLI and Shifter CLI installed onto your workstation. This illustration is run on Mac OS to show shifter tool could be used cross-platform. In fact, shifter supports [MAC/Linux/Windows starting from 0.12](https://github.com/google/shifter/releases).
...
mv velero-v1.9.2-linux-amd64/velero ./velero
chmod +x velero
```
Also set the PATH to access shifter or move it to a place where the PATH is already set.

We recommend conducting migration processes per namespace instead of the entire cluster. Also, a switch downtime is expected for the migration process, which normally can be minutes to hours, depending on the workload types. The following diagram provides a quick overview

### Create a S3 Bucket in AWS and Use Velero with S3 plugin to backup the cluster

[velero-plugin-for-aws](https://github.com/vmware-tanzu/velero-plugin-for-aws) supports:
...
In this walk-through, we choose opt-out since we intended to backup all persistent volumes in the desired namespace. By adding "--default-volumes-to-restic" in the backup command could easily activate the opt-out mode. Compared with the opt-in mode which requires additional annotations, opt-out mode provides an easier way for existing openshift data migration.

### Converting Manifests
Capture openshift objects into YAML format and stored in ocp-manifests/ folder. Then, shifter is applied to convert those openshift objects into standard kubernetes object yaml files and stored in kubernetes-manifests/ folder (shown in the last step).
...
# Other Considerations and object/resource migration

Apart from the integral objects discussed above there are other object that need to be ported from OpenShift to Kubernetes. Based on the cluster composition consider migrating the objects below.


### 1. Migrating ClusterResourceQuotas
...
## 3. Migrating Project Specific Resource Quotas
...
## 4. Migrate Cluster Roles and Cluster Role Bindings
...
### 5. Migrate Project Roles, Service Accounts and RoleBindings
